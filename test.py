...
def extract_features(x):
    return {"letter": x[1], "pixels": x[6:134]}
    ...
    for i in range(200):
        ...
        if i % 100 == 0:
            ...
            train_accuracy = accuracy.eval(
                feed_dict={x: batch_examples, y_: batch_labels, keep_prob: 1.0}
            )
            print("step %d, training accuracy %g" % (i, train_accuracy))
        sess.run(
            train_step, feed_dict={x: batch_examples, y_: batch_labels, keep_prob: 0.5}
        )

    c_preds = correct_prediction.eval(feed_dict={x: images, y_: labels, keep_prob: 1.0})
    for itr, cp in enumerate(c_preds):
        c_accs = accuracy.eval(
            feed_dict={
                x: images[itr : itr + 1],
                y_: labels[itr : itr + 1],
                keep_prob: 0.7,
        ...
        print(
            f"error: {1 - c_accs}\n. Original: {pos_to_char(labels[itr].tolist().index(1))}, Prediction: {pos_to_char(pred[0])}"
        )
